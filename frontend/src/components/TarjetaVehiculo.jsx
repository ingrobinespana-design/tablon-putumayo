...
export default function TarjetaVehiculo({ pub }) {
  ...
  const tipo = LABEL_VEHICULO_TIPO[a.tipo] || 'Vehículo';
    ...
    <Link to={`/vehiculo/${pub.id}`} style={estilos.tarjeta}>
      ...
      <div style={estilos.cuerpo}>
        ...
        <div style={estilos.meta}>
          ...
          {a.kilometraje != null && a.kilometraje !== '' && <span>· {fmt(a.kilometraje)} km</span>}
        </div>
        {pub.zona && (
          <div style={estilos.zona}><MapPin size={14} /><span>{pub.zona}</span></div>
        )}
        {pub.precio_esperado && (
          <div style={estilos.precio}>
            Precio: <strong>${fmt(pub.precio_esperado)}</strong>
          </div>
        )}
      </div>
    </Link>
...
const estilos = {
  tarjeta: {
    display: 'block', background: 'var(--crema-card)', borderRadius: 'var(--radius)',
    overflow: 'hidden', boxShadow: 'var(--sombra)', border: '1px solid var(--linea)',
  },
  ...
  selloTipo: {
    position: 'absolute', bottom: '10px', left: '10px', background: 'rgba(0,0,0,0.62)',
    color: 'white', fontSize: '12px', fontWeight: 600, padding: '3px 9px', borderRadius: '999px',
  },
  contadorFotos: {
  ...
  cuerpo: { padding: '16px' },
  titulo: { fontSize: '18px', marginBottom: '4px' },
  meta: { fontSize: '13.5px', color: 'var(--carbon-suave)', marginBottom: '8px' },

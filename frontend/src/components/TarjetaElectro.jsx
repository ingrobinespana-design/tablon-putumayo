...
export default function TarjetaElectro({ pub }) {
  ...
  const tipo = LABEL_ELECTRO_TIPO[a.tipo] || 'Electrodoméstico';
    ...
    <Link to={`/electrodomestico/${pub.id}`} style={estilos.tarjeta}>
      ...
      <div style={estilos.cuerpo}>
        ...
        <div style={estilos.meta}>
          {a.estado && <span style={estilos.estado}>{a.estado === 'nuevo' ? 'Nuevo' : 'Usado'}</span>}
          {a.capacidad && <span>· {a.capacidad}</span>}
        </div>
...
const estilos = {
  ...
  estado: { fontWeight: 700, color: 'var(--terracota)' },

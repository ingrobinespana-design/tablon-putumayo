...
import { comisionDe, desgloseComision, LABEL_VARIOS_GRUPO } from '../config/catalogo';
...
export default function DetalleVarios() {
  const { id } = useParams();
  ...
  useEffect(() => {
    api.obtenerPublicacion(id)
      .then(setPub)
      .catch(() => setError('No encontramos este aviso. Puede que ya se haya vendido o el enlace esté mal.'))
  ...
  if (error && !pub) {
    return (
      <div className="contenedor" style={{ padding: '40px 20px' }}>
        ...
        <Link to="/varios" className="btn btn-secundario" style={{ marginTop: '16px' }}>
  ...
  const grupo = a.grupo ? (LABEL_VARIOS_GRUPO[a.grupo] || a.grupo) : null;
  ...
  return (
    <div className="contenedor" style={{ padding: '28px 20px 60px' }}>
      <Link to="/varios" style={estilos.volver}><ArrowLeft size={16} /> Volver</Link>
      ...
      <div className="detalle-layout">
        ...
        <div>
          ...
          {grupo && <p style={estilos.tipo}>📦 {grupo}</p>}

          {(pub.zona || pub.descripcion) && (
            <div style={estilos.datos}>
              {pub.zona && <Dato label={<><MapPin size={13} /> Zona</>} valor={pub.zona} />}
              {grupo && <Dato label="Categoría" valor={grupo} />}
            </div>
          )}
          ...
          {pub.estado !== 'vendido' && (
            <div style={estilos.botones}>
              ...
              <BotonCompartir titulo={pub.titulo} precio={pub.precio_esperado} ruta={`/articulo/${pub.id}`} estilo={{ flex: 1, padding: '14px' }} />
            </div>
          )}

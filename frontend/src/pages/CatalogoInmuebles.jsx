...
import { INMUEBLE_TIPOS } from '../config/catalogo';

export default function CatalogoInmuebles() {
  const POR_PAGINA = 12;
  ...
  async function cargar() {
    ...
    try {
      const datos = await api.listarPublicaciones('inmuebles', { zona: filtroZona || undefined });
      setPubs(datos);
    } catch (e) {
      setError('No pudimos cargar los inmuebles. Intenta de nuevo en un momento.');
    } finally {
      clearTimeout(aviso);
      setDespertando(false);
      setCargando(false);
  ...
  return (
    <div className="contenedor" style={{ padding: '32px 20px 60px' }}>
      <div style={estilos.encabezado}>
        <div>
          <h1 style={estilos.h1}>Inmuebles</h1>
          <p style={estilos.sub}>Casas, fincas, lotes y locales en el Putumayo.</p>
        </div>
        <Link to="/publicar/inmuebles" className="btn btn-primario">
      ...
      {!cargando && !error && filtrados.length === 0 && (
        <div style={estilos.vacio}>
          <h3 style={estilos.vacioTitulo}>Todavía no hay inmuebles publicados aquí</h3>
          <p>Sé el primero — publica tu casa, finca o lote y empieza a recibir ofertas.</p>
        </div>
      )}
      ...
      {!cargando && filtrados.length > 0 && (
        <>
          <div className="catalogo-grid">
            {filtrados.slice(0, visibles).map((pub) => (<TarjetaInmueble key={pub.id} pub={pub} />))}
          </div>
          {visibles < filtrados.length && (
            <div style={estilos.verMasCaja}>
              <button className="btn btn-secundario" onClick={() => setVisibles((v) => v + POR_PAGINA)}>
                Ver más inmuebles ({filtrados.length - visibles} restantes)
              </button>
            </div>
          )}
        </>

...
import TarjetaElectro from '../components/TarjetaElectro';
import { ELECTRO_TIPOS } from '../config/electro_data';

export default function CatalogoElectro() {
  const POR_PAGINA = 12;
  ...
  async function cargar() {
    ...
    try {
      const datos = await api.listarPublicaciones('electrodomesticos', { zona: filtroZona || undefined });
      setPubs(datos);
    } catch (e) {
      setError('No pudimos cargar los electrodomésticos. Intenta de nuevo en un momento.');
    } finally {
      clearTimeout(aviso);
      setDespertando(false);
      setCargando(false);
  ...
  return (
    <div className="contenedor" style={{ padding: '32px 20px 60px' }}>
      <div style={estilos.encabezado}>
        <div>
          <h1 style={estilos.h1}>Electrodomésticos</h1>
          <p style={estilos.sub}>Neveras, TVs, lavadoras, celulares y más en el Putumayo.</p>
        </div>
        <Link to="/publicar/electrodomesticos" className="btn btn-primario">
      ...
      {!cargando && !error && filtrados.length === 0 && (
        <div style={estilos.vacio}>
          <h3 style={estilos.vacioTitulo}>Todavía no hay electrodomésticos publicados aquí</h3>
          <p>Sé el primero — publica tu TV, nevera o lavadora y empieza a recibir ofertas.</p>
        </div>
      )}
      ...
      {!cargando && filtrados.length > 0 && (
        <>
          <div className="catalogo-grid">
            {filtrados.slice(0, visibles).map((pub) => (<TarjetaElectro key={pub.id} pub={pub} />))}
          </div>

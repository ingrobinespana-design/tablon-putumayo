...
export default function PublicarElectro() {
  ...
  const [form, setForm] = useState({
    ...
    estado_articulo: 'usado', descripcion: '',
  ...
  function cambiarTipo(v) { setForm((p) => ({ ...p, tipo: v, marcaSel: '', marcaOtra: '' })); }

  const marcas = marcasElectroDe(form.tipo);
  ...
  async function manejarEnviar(e) {
    e.preventDefault();
    setError(null);
    if (!marcaEfectiva) { setError('Elige (o escribe) la marca.'); return; }

    const tipoLabel = LABEL_ELECTRO_TIPO[form.tipo] || 'Electrodoméstico';
    const titulo = [tipoLabel, marcaEfectiva, form.referencia.trim(), form.capacidad.trim()]
    ...
    try {
      const atributos = {
        ...
        estado: form.estado_articulo,
      };
      const fd = new FormData();
      fd.append('categoria', 'electrodomesticos');
    ...
    return (
      <div className="contenedor" style={{ padding: '60px 20px', maxWidth: '560px' }}>
        <div style={estilos.exitoBox}>
          ...
          <h2 style={{ marginTop: '16px' }}>Tu artículo quedó en revisión</h2>
  ...
  return (
    <div className="contenedor" style={{ padding: '32px 20px 60px', maxWidth: '660px' }}>
      <h1 style={estilos.titulo}>Publica tu electrodoméstico</h1>
      <p style={estilos.subtitulo}>Entre más fotos y datos, más rápido se vende. Revisamos cada publicación antes de mostrarla.</p>

      {error && <p style={estilos.errorTexto}>{error}</p>}

      <form onSubmit={manejarEnviar} style={estilos.form}>
        <div style={estilos.bloque}>
          <h3 style={estilos.bloqueTitulo}>Datos del artículo</h3>

          <label style={estilos.label}>
            Tipo *
            <select value={form.tipo} onChange={(e) => cambiarTipo(e.target.value)} style={estilos.input}>
              {ELECTRO_TIPOS.map((t) => (<option key={t.valor} value={t.valor}>{t.label}</option>))}
            </select>
          </label>
          ...
          <div style={estilos.fila}>
            <label style={estilos.label}>
              Marca *
              <select value={form.marcaSel} onChange={(e) => actualizar('marcaSel', e.target.value === OTRA ? OTRA : e.target.value)} required style={estilos.input}>
            ...
            <label style={estilos.label}>
              Referencia / modelo
              <input type="text" value={form.referencia} onChange={(e) => actualizar('referencia', e.target.value)}
                placeholder="Ej: UN55AU7000" style={estilos.input} />
            </label>
          </div>
          ...
          <div style={estilos.fila}>
            <label style={estilos.label}>
              Capacidad / tamaño
              <input type="text" value={form.capacidad} onChange={(e) => actualizar('capacidad', e.target.value)}
                placeholder={CAP_ELECTRO_TIPO[form.tipo] || 'Capacidad / tamaño'} style={estilos.input} />
            </label>
            <label style={estilos.label}>
              Estado *
              <select value={form.estado_articulo} onChange={(e) => actualizar('estado_articulo', e.target.value)} style={estilos.input}>
                {ELECTRO_ESTADOS.map((s) => (<option key={s.valor} value={s.valor}>{s.label}</option>))}
              </select>
            </label>
          </div>
          ...
          <label style={estilos.label}>
            ...
            <textarea value={form.descripcion} onChange={(e) => actualizar('descripcion', e.target.value)}
              rows={4} maxLength={2000} placeholder="Estado, tiempo de uso, si tiene factura/garantía, motivo de venta…"
        ...
        <div style={estilos.bloque}>
          ...
          <p style={estilos.bloqueNota}>Sube varias: de frente, marca/referencia, detalles, funcionando. A cada foto ponle un pie. Máximo {MAX_FOTOS}.</p>
